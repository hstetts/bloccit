...
RSpec.describe VotesController, type: :controller do
  ...
  #test that unsigned-in users are redirected to the sign-in page, as they will not be allowed to vote on posts.
  context "guest" do
  ...
  #create a context to test signed-in users, who should be allowed to vote on posts.
  context "signed in user" do
    ...
    describe "POST up_vote" do
     #expect that the first time a user up votes a post, a new vote is created for the post.
     ...
     #test that a new vote is not created if the user repeatedly up votes a post.
     ...
     #expect that up voting a post will increase the number of points on the post by one.
    ...
    #test to ensure users are redirected back to the correct view (posts show or topics show) depending on view they up voted from.
    #We do this by setting request.env["HTTP_REFERER"] to the requesting URL.
